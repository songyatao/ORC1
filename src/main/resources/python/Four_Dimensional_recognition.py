...
def annotate_and_plot_image(image_path, threshold_distance=50, save_path=None, font_size=12):
    try:
        ...
        font_path = 'D:\\SWork\\SimHei.ttf'  # Windows font path
...
def process_images_in_folder(folder_path, threshold_distance=50, font_size=12):
    # Iterate over each subfolder in the top-level directory
    for subfolder_name in os.listdir(folder_path):
        subfolder_path = os.path.join(folder_path, subfolder_name)

        if os.path.isdir(subfolder_path):  # Check if it's a directory
            # Ensure output folder exists within each subfolder
            output_folder = os.path.join(subfolder_path, 'four_dimensional')
            ...
            for filename in os.listdir(subfolder_path):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    # Define save path for annotated images within the same subfolder
                    save_path = os.path.join(output_folder, f"{os.path.splitext(filename)[0]}_four.jpg")
                    annotate_and_plot_image(image_path, threshold_distance, save_path, font_size)

# Usage example
if __name__ == "__main__":
    folder_path = sys.argv[1]
    process_images_in_folder(folder_path, threshold_distance=50, font_size=14)
