...
# 遍历 base_folder 目录下的所有文件夹。
# 对于每个子文件夹，使用 glob.glob 查找其中的所有 .jpg 文件。
# 将找到的图片文件路径添加到 image_files 列表中。
# 返回包含所有找到的图片文件路径的列表。
...
def process_image(image_file, font_path):
    ...
    #保存
    directory = os.path.dirname(image_file) + "\\two_dimensional"
    ...
    directory = os.path.dirname(image_file) + "\\two_dimensional\\" + os.path.basename(image_file)
    # print("555")
    # 保存输出图像
    output_path = os.path.splitext(directory)[0] + '_annotated.png'  # 保存为PNG以保留透明度
    # print("6666")
    try:
...
def main():
    ...
    font_path = "D:\\SWork\\SimHei.ttf"  # 替换为你的字体文件路径
    image_files = read(base_folder) # 返回的是所有找到的图片文件路径的列表
